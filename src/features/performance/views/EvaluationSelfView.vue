...
async function openModalHandler(row) {
  ...
  try {
    const { detail, factorScores } = await getSelfEvaluationDetail(row.resultId);
...
async function handleDownload() {
  try {
    ...
    link.href = url;
    link.setAttribute('download', `evaluation-self-list-${new Date().toISOString().slice(0, 10)}.xlsx`);
